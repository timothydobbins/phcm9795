...
```{r fig1-1, echo=FALSE, out.width = "100%"}
...
```{r fig1-2, echo=FALSE, out.width = "66%", fig.cap="Characteristics of the sample of prisoners from the NPHDC", fig.pos="H"}
...
huxtable::huxtable(tab1_ages, add_colnames = FALSE) %>%
  ...
  theme_article() %>% 
  set_width(0.8) %>% 
  set_caption("Age of 30 gym attendees")
```
...
```{r tab1-ages-freq1, echo=FALSE, fig.pos="H"}
tab1_2 <- tribble(
  ~Age, ~Frequency,
"17",	1L,
"18",	5L,
...
huxtable(tab1_2) %>%
  ...
  theme_article() %>% 
  set_width(0.6) %>% 
  set_caption("Frequency of ages of students visiting a gym")
```
...
```{r tab1-ages-freq2, echo=FALSE, fig.pos="H"}
tab1_3 <- tribble(
...
huxtable(tab1_4) %>%
  ...
  theme_article() %>% 
  set_width(0.9) %>% 
  set_caption("Frequency of sex of students visiting a gym")
```
...
```{r tab1-sex-bmi-freq, echo=FALSE}
...
huxtable(tab1_5) %>%
  ...
  theme_article() %>% 
  set_width(0.9) %>% 
  set_caption("Frequency of students visiting a gym by sex and BMI status*") %>% 
...
```{r fig-1-1, echo=FALSE, out.width = "75%", fig.cap="Bar graph of stage of disease from PBC study", fig.pos="H"}
...
```{r fig-1-2, echo=FALSE, out.width = "75%", fig.cap="Bar graph of stage of disease by sex from PBC study", fig.pos="H"}
...
```{r fig-1-3, echo=FALSE, out.width = "75%", fig.cap="Stacked bar graph of stage of disease by sex from PBC study", fig.pos="H"}
...
```{r fig-1-4, echo=FALSE, out.width = "75%", fig.cap="Relative frequency of sex within stage of disease from PBC study", fig.pos="H"}
...
```{r fig-1-5, echo=FALSE, out.width="66%", fig.pos="H", fig.cap="Prostate cancer age-standardised incidence and mortality rates (per 100,000), NSW, 1972-2014"}
...
```{r fig-1-6, echo=FALSE, out.width = "100%", fig.cap="Histogram of age (left) and serum bilirubin (right) from PBC study data"}
...
```{r fig-1-7, echo=FALSE, out.width = "100%", fig.cap="Box plot of age (left) and serum bilirubin (right) from PBC study data"}
...
```{r mod03-z-t-dist, echo=FALSE, out.width = "100%", fig.cap="The normal (Z) and the student’s t-distribution with 2, 5 and 30 degrees of freedom"}
...
huxtable(tab4_1) %>%
  theme_article() %>% 
  set_width(0.8) %>% 
  set_caption("Interpretation of P-values")
```
...
```{r practical-importance, echo=FALSE, out.width = "66%", fig.cap="Statistical significance vs. clinical importance (Source: Armitage P, Berry G, Matthews JNS. (2001)"}
knitr::include_graphics(here::here("img", "mod04", "practical-vs-statistical-sig.png"))
```
...
if (knitr::is_html_output()) {
  ...
  huxtable(df) %>%
    ...
    set_caption("Comparison of study result with the truth") %>% 
...
```{r cis-overlapping-null, echo=FALSE, out.width = "66%", fig.cap="Using confidence intervals as informal hypothesis tests"}
knitr::include_graphics(here::here("img", "mod04", "cis-overlapping-null.png"))
```
...
huxtable(df) %>%
add_rows(c(NA, NA, "(indicating no difference)"), after=1) %>%
  merge_repeated_rows(everywhere, 1) %>% 
  theme_article() %>% 
  set_caption("Values indicating no effect")
```
...
```{r two-tailed-test, echo=FALSE, out.width = "66%", fig.cap="P-value for a 2-tailed test"}
...
```{r one-tailed-tests, echo=FALSE, out.width = "90%", fig.cap="P-value for 1-tailed tests"}
knitr::include_graphics(here::here("img", "mod04", "one-tailed-tests.png"))
```
...
```{r mod09-los, echo=FALSE, out.width = "66%", fig.cap="Length of hospital stay for 132 patients"}
...
```{r mod09-lnlos, echo=FALSE, out.width = "66%", fig.cap="istribution of log transformed (length of stay + 1)"}
...
huxtable(df) %>%
  ...
  theme_article() %>% 
  set_width(0.95) %>% 
  set_caption("Transforming data to ranks: first 21 participants")
```
...
huxtable(df) %>%
  ...
  theme_article() %>% 
  set_width(0.8) %>% 
  set_caption("Arthritis symptom scores for 11 patients after administering two drugs")
```
...
```{r mod09-diff-hist, echo=FALSE, out.width = "66%", fig.cap="Distribution of difference in symptom scores between Drug 1 and Drug 2"}
...
huxtable(select(tab10_1, -P)) %>%
  ...
  set_width(0.95) %>% 
  set_caption("Sample size required to calculate a 95\\% confidence interval with a given precision")
```
